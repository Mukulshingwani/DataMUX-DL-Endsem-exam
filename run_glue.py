...
task_to_keys = {
    "cola": ("sentence", None),
    "mnli": ("premise", "hypothesis"),  # our
    "mrpc": ("sentence1", "sentence2"),
    "qnli": ("question", "sentence"),
    "qqp": ("question1", "question2"), # our
...
class ModelArguments:
    ...
    binary_hadamard_epsilon: Optional[float] = field(
        default=0,
        metadata={"help": "Percentage intersection among binary vectors, default is no intersection"},
    )
...
def main():
    ...
    if (
        ...
        last_checkpoint = get_last_checkpoint_trainerstate_robust(training_args.output_dir)
    ...
    if data_args.task_name is not None:
        ...
        if model_args.retrieval_pretraining:
            # add dummy value here
            num_labels = 2
            is_regression = False
            label_list = ["dummy1", "dummy2"]
        else:
    ...
    if model_args.retrieval_pretraining:
        ...
        def group_texts(examples):
            ...
            len_result = len(result[random_key])
            result["label"] = [random.randint(0, num_labels-1) for _ in range(len_result)]
            return result
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            metrics = trainer.evaluate(eval_dataset=eval_dataset, speed_metrics=True)
